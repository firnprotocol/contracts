...
contract Firn is EpochTree {
    ...
    uint32 constant _fee = 128; // fee will be floor(amount / 128)
    ...
    struct Info {
        // try to save storage space by using smaller int types here
        uint64 epoch;
    ...
    function lengths(uint64 epoch) external view returns (uint256) {
        // see https://ethereum.stackexchange.com/a/20838.
        return lists[epoch].length;
    }

    constructor(DepositVerifier deposit_, TransferVerifier transfer_, WithdrawalVerifier withdrawal_, address treasury_) {
        _deposit = deposit_;
        _transfer = transfer_;
        _withdrawal = withdrawal_;
        _treasury = treasury_;
    ...
    function touch(bytes32 Y, uint32 credit, uint64 epoch) internal {
        ...
        if (current.epoch > 0) {
            // will only be false for registration...?
